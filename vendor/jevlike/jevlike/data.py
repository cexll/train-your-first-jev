...
def synthetic_example(seed: int) -> ChoiceExample:
    ...
    # sorted(), not list(): a set of strings iterates in hash order, which changes
    # with PYTHONHASHSEED, so list(options) made the option order and every label
    # differ between processes for the same seed. The set is unchanged otherwise.
